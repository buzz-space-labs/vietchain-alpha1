...
func TestGenesisState_Validate(t *testing.T) {
	for _, tc := range []struct {
		...
		},
		{
			...
			genState: &types.GenesisState{
				Params: types.Params{
					ExcludeCirculatingAddr: []string{"addr1", "annam1jlp9ge244um2v7mdm7xwamwsv9z9vhpej6wjh7"},
					MaxSupply:              types.DefaultParams().MaxSupply,
				},
			},
			valid: false,
		},
		{
			...
			genState: &types.GenesisState{
				Params: types.Params{
					ExcludeCirculatingAddr: []string{"annam19ad4tprcf9ew4577qph3jfzpf9slcrkpmxwvah", "annam1jlp9ge244um2v7mdm7xwamwsv9z9vhpej6wjh7"},
					MaxSupply:              types.DefaultParams().MaxSupply,
				},
			},
			valid: true,
		...
		t.Run(tc.desc, func(t *testing.T) {
			sdk.GetConfig().SetBech32PrefixForAccount("annam", "annampub")
